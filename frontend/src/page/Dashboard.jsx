    ...
    try {
      ...
      if (res.ok) {
        setMembers(prev => prev.map(m => 
          m.id === selectedMember ? { ...m, isSubmitted: true, pdVerified: false } : m
        ));
        setHomeImage(null); 
        setSideImage(null);
        setSelectedMember('');
        ...
        alert(`Server Error: ${data.error || data.message || 'Unknown'}`);
      }
    } catch (err) { 
                ...
                members.map(m => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      {m.isSubmitted && (
                        <div className={`flex flex-col items-center justify-center p-2 rounded-xl min-w-[70px] ${m.pdVerified ? 'text-emerald-400 bg-emerald-500/10' : 'text-amber-400 bg-amber-500/10'}`} title={m.pdVerified ? "Approved" : "Pending Verification"}>
                          {m.pdVerified ? (
                            <><FaCheckCircle className="text-xl" /><span className="text-[8px] font-black mt-1 uppercase">APPROVED</span></>
                          ) : (
                            <><FaSyncAlt className="text-xl animate-spin-slow" /><span className="text-[8px] font-black mt-1 uppercase">PENDING</span></>
                          )}
                        </div>
                      )}
                    </div>
